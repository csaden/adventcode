...
# While you consider checking the in-flight menu for one of those drinks that come with
# a little umbrella, you are interrupted by the kid sitting next to you.
...
# Then, the accumulator is increased from 0 to 1 (acc +1) and jmp +4 sets the next
# instruction to the other acc +1 near the bottom. After it increases the accumulator
# from 1 to 2, jmp -4 executes, setting the next instruction to the only acc +3.
...
def process_step(step):
  command, num = step.split(" ")
  return command, int(num)

def do_boot_code(lines):
  i = 0
  acc = 0
  seen = set()
  ...
  while i not in seen:
    ...
    command, num = process_step(lines[i])
...
def swap(lines, indices, command, swap):
  for i in indices:
    swapped = lines.copy()
    swapped[i] = swapped[i].replace(command, swap)
    acc, terminates = do_boot_code(swapped)
    if terminates:
      return acc
  return None

if __name__ == "__main__":
  lines = read_file("day08_input.txt")
  acc, terminates = do_boot_code(lines)
  print('Part 1 accumulator value before any instruction is executed twice', acc)

  jmp_indices = [i for (i, line) in enumerate(lines) if line.startswith(JMP)]
  nop_indices = [i for (i, line) in enumerate(lines) if line.startswith(NOP)]

  res = swap(lines, jmp_indices, JMP, NOP) or swap(lines, nop_indices, NOP, JMP)
  print('Part 2 accumulator value after swapping command with terminating code', res)
