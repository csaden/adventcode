...
def count_any_yes(lines):
  s = set()
  for line in lines:
    s.update(line)
  return len(s)

def count_all_yes(lines):
  sets = [set(line) for line in lines]
  return len(set.intersection(*sets))

if __name__ == '__main__':
  lines = read_file('day06_input.txt')
  ...
  part1 += count_any_yes(group)
  part2 += count_all_yes(group)
  print('Part 1 number of questions to which anyone answered yes', part1)
  print('Part 2 number of questions to which all answered yes', part2)
