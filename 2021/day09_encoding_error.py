...
def no_two_sum_in_window(arr, window=25):
  for i in range(0, len(arr) - window):
    if not has_two_sum(arr[i:i+window], arr[i+window]):
      return arr[i+window]
  return None

def find_contiguous_sum(arr, target_sum=542529149):
  for i in range(len(arr)):
    curr_sum = arr[i]
    j = i + 1
    while curr_sum < target_sum and j < len(arr):
      curr_sum += arr[j]
      j += 1
...
if __name__ == "__main__":
  lines = read_file("day09_input.txt")
  arr = [int(l) for l in lines]

  invalid_num = no_two_sum_in_window(arr)
  print('Part 1 number that has no two sum in last 25 numbers', invalid_num)

  encrypted_weakness = find_contiguous_sum(arr)
  print('Part 2 encryption weakness', encrypted_weakness)
