...
# is not; it contains no instances of b, but needs at least 1. The first and third passwords
...
def is_valid_password(password, letter, lo, hi):
  """Password must have the letter occuring at least lo times and no more than hi times """
  counter = Counter(password)
  return counter[letter] >= lo and counter[letter] <= hi

def is_valid_corporate_password(password, letter, i, j):
  """Password must have exactly one occurrence of the letter at index i or j"""
...
def process_line(line):
  ...
  [lo, hi] = [int(num) for num in counts.split("-")]
  letter = char[0]
  return password, letter, lo, hi


if __name__ == '__main__':
  lines = read_file('day02_input.txt')
  valid_passwords = sum([1 if is_valid_password(*process_line(line)) else 0 for line in lines])
  print('Valid passwords', valid_passwords)

  valid_passwords = sum([1 if is_valid_corporate_password(*process_line(line)) else 0 for line in lines])
  print('Valid corporate passwords', valid_passwords)
