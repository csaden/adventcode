...
def count_trees(grid, rise = 1, run = 3):
  # length of each line in the grid / landscape
  m = len(grid[0]) - 1
  # max number of lines in grid
  n = len(grid)
  # index to track the column in the grid
  i = 0
  # index to track the row in the grid
  j = 0
  # counter to track the trees encountered
  trees = 0
  while j + rise <= n:
    if grid[j][i] == TREE:
      trees += 1
...
if __name__ == '__main__':
  lines = read_file("day03_input.txt")
  trees = count_trees(lines)
  print(trees)

  # part two
  run_1_trees = count_trees(lines, rise=1, run=1)
  run_2_trees = count_trees(lines, rise=1, run=3)
  run_3_trees = count_trees(lines, rise=1, run=5)
  run_4_trees = count_trees(lines, rise=1, run=7)
  run_5_trees = count_trees(lines, rise=2, run=1)
  print(run_1_trees * run_2_trees * run_3_trees * run_4_trees * run_5_trees)
