...
const getIngredients = function(file) {
    let input = fs.readFileSync(file).toString();
    input = input.split('\n');
    return input.map(function(line) {
        let name = line.split(':')[0];
        let props = line.match(/-?\d+/g);
        props = props.map(function(strNum) {
            return parseInt(strNum, 10);
        });
        return new Ingredient(name, ...props);
    });
};

const Ingredient = function(name, capacity, durability, flavor, texture, calories) {
    this.name = name;
...
const calcPropertyScore = function(prop, ingredients) {
    const propertyScore = ingredients.reduce(function(prev, curr) {
        prev += curr[prop] * curr.amount;
        return prev;
    }, 0);
    return propertyScore <= 0 ? 0 : propertyScore;
};

const calcPropertiesScore = function(ingredients) {
    ...
    for (let p in propertyScores) {
        propertyScores[p] = calcPropertyScore(p, ingredients);
    }
    return propertyScores;
}

const calcTotalScore = function(propertyScores) {
    let totalScore = 1;
    for (let p in propertyScores) {
        totalScore *= propertyScores[p];
    }
    return totalScore;
};

const countCalories = function(ingredients) {
    return ingredients.reduce(function(prev, curr) {
        prev += curr.calories * curr.amount;
        return prev;
    }, 0);
};

const main = function(isCountingCalories) {
    const ingredients = getIngredients('puzzle15_data.txt');
    let maxScore = 0, currScore;
    for (let i = 0; i <= 100; i++) {
        for (let j = 0; j <= 100 - i; j++) {
            for (let k = 0; k <= 100 - i - j; k++) {
                ...
                currScore = calcTotalScore(calcPropertiesScore(ingredients));
                if (isCountingCalories && countCalories(ingredients) !== 500) {
                    continue
                }
