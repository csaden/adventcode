...
# the adapters, and your device. What is the number of 1-jolt differences multiplied
...
def count_jolt_diffs(arr):
  diffs = Counter()
  current_adapter = arr[0]
  for i in range(1, len(arr)):
    diff = arr[i] - current_adapter
    diffs[diff] += 1
    current_adapter = arr[i]

  return diffs[1] * diffs[3]

def count_configurations(adapters):
  configs = defaultdict(int, {0: 1})
  for adapter in adapters[1:]:
    configs[adapter] = configs[adapter-1] + configs[adapter-2] + configs[adapter-3]
  maximum_jolt = adapters[-1]
  return configs[maximum_jolt]


if __name__ == "__main__":
  lines = read_file("day10_input.txt")
  adapters = [int(l) for l in lines]
  adapters.insert(0, 0) # insert 0 jolt for outlet
  adapters.sort()
  adapters.append(adapters[-1] + 3) # add one more jolt diff of 3 for device's built in adapter

  part1 = count_jolt_diffs(adapters)
  print('Part 1 product of number of 1-jolt and 3-jolt diffs', part1)

  part2 = count_configurations(adapters)
  print('Part 2 number of distinct ways to arrange adapters to outlet', part2)
